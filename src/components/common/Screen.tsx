...
const Screen = (props: ScreenProps) => {
  ...
  const { enableSafeArea, withLoadingView, showLoading, navBar, enableKeyboardAvoidingView = false } = props;
  ...
  NestedEle = withLoadingView ? (
    <View style={[styles.container]}>
      ...
      {withLoadingView && (showLoading || !completeHideLoadingView) && (
        <Animated.View style={[styles.loadingView, { opacity: fadeInAnimRef.current }]}>
          <LoadingView dotSize={32} containerStyle={styles.loadingIndicatorStyle} />
        </Animated.View>
      )}
    </View>
  ) : (
